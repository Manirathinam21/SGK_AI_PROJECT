...
classifier=joblib.load('/Users/manirathinams/Documents/KT/Magnum_pdf/Magnum.sav')
# ------------------------------------------------------------------------------------------------
##Extracting each page tables using pdfplumber package

def tables_extraction(file_path,pgs):
    with pdfplumber.open(file_path) as pdf:
        first_page = pdf.pages[pgs-1]
...
def key_values(dataframes):
    ...
    return key, val
# -----------------------------------------------------------------------------------------------------------
# slicing of content classification parts

def content_classification_slicing(file_path, pgs):
    global a, b
    a=0 
    b=0
    from pdf2docx import parse
    word_file='/Users/manirathinams/Documents/KT/Magnum_pdf/Magum/Magnum.docx'
    ...
    return text
# --------------------------------------------------------------------------------------------------------------
# content classification key_value pair
...
def Content_classification(text):
    ...
    return key1, val1
# -----------------------------------------------------------------------------------------------------------------
# Nutrition part slicing 

def Nutrition_slicing(file_path,pgs):
    #pgs=0
...
def net_content(nutr_hd):    
    ...
    return nut1
# ------------------------------------------------------------------------------------------------
# General content main function

def general_main(file_path, pgs):
    dataframes=tables_extraction(file_path, pgs)
    key,val=key_values(dataframes)
    text=content_classification_slicing(file_path, pgs)
    ...
    return general
# -----------------------------------------------------------------------------------------------------
# Nutrition content main function

def Nutrition_main(file_path,pgs):
    nutr, nutr_hd=Nutrition_slicing(file_path,pgs)
    ...
    return Netcontant
# ----------------------------------------------------------------------------------------------------------
# magnum_pdf main function

def magnum_main(file_path, pages):
    ...
    for pgs in page_no:
        try:  
            ...
            output=Nutrition_main(file_path, pgs)
            all_pages[str(pgs)]=output
