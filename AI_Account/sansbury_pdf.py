...
classifier=joblib.load('/Users/manirathinams/Documents/Python /sansbury pdf/sansbury_nutri.sav')
mlp_model=joblib.load('/Users/manirathinams/Documents/Python /sansbury pdf/woolworths.sav')
# ------------------------------------------------------------------------------------------------
...
def data_extraction_table(file_path, pgs):
    ...
    parse(file_path, word_file, pages=[int(pgs-1)])
...
#Here it will get values from col_row_span, 1st IF condn to get serial_no, 2nd ELIF condn to ALLERGEN, INGREDIENTS, 3rd ELSE for UNMAPPED keys& values
        ...
        if prob[-1]>=0.95 and classified_output=="SERIAL_NUMBER":
            ...
            if str(col_row_span[x].lower().strip()).replace('<b>','').replace('</b>','').replace('\n','') not in unwanted_text:
                col_row_text=str(col_row_span[x]).replace('<','&lt;').replace('>','&gt:').replace('\n','').strip()
                classified_output='UNMAPPED'
                if classified_output in general_dict:
                    general_dict[classified_output].append({classify(col_row_text)[0]:str(col_row_text)})
                else:
                    general_dict[classified_output]=[{classify(col_row_text)[0]:str(col_row_text)}]                
    keys=key1+key2
    val=val1+val2
    ...
    return general_dict
#----------------------------------------------------------------------------------------------------------------
#Back of the pack Nutrition Extraction Functions from multiple tables in a page using Camelot
...
def bop_nutrition_extraction(file_path,pgs):
    ...
    return dfs
#----------------------------------------------------------------------------------------------------------------
#Front of the pack Nutrition table Extraction functions from multiple tables in a page using Camelot
...
#Removing the duplicate keys
    ...
    return nutr
#------------------------------------------------------------------------------------------------------------
#Servings and Nutrition header extraction
...
#Extracting only the serving contents from dataframe by sorting len of row <2 and len of text in each row <20
    ...
    #Removing the duplicate keys
        ...
        return {}
#---------------------------------------------------------------------------------------------------------------------
#writing a main function for general information part    
...
def general_main(file_path, pgs):
    ...
    if serve:
        ...
        return general
#------------------------------------------------------------------------------------------------
#writing a main function for nutritional information part 
...
def nutrition_main(file_path, pgs):   
    ...
    return fnl
#------------------------------------------------------------------------------------------------
#Sansbury_pdf Main Function
def sansbury_main(file_path,pages):
    page_no= [int(x) for x in pages.split(",")]
    all_pages={}
    for pgs in page_no:
